import pandas as pd
parkinson_data=pd.read_csv('/content/pd_speech_features1.csv')#link to dataset is given in readme file
...
sel = SelectFromModel(ExtraTreesClassifier(criterion='entropy'))
...
for col in rfe:
    if col not in selected_feat:
      del X_Test[col]
ni1 = ExtraTreesClassifier(criterion = 'entropy')
